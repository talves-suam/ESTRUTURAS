...
try {
    if ($method === 'GET') {
        ...
        if ($id !== '') {
            $st = $pdo->prepare('SELECT payload FROM curriculum_courses WHERE id = ? LIMIT 1');
            $st->execute([$id]);
            $row = $st->fetch();
            $item = $row ? api_decode_payload($row['payload'] ?? null) : null;
            if (!$item) {
                api_fail(404, 'Curso não encontrado.');
            }
            api_ok($item);
        }

        $st = $pdo->query('SELECT payload FROM curriculum_courses ORDER BY name ASC, code ASC');
        $list = [];
        while ($row = $st->fetch()) {
    ...
    if ($method === 'POST' || $method === 'PUT') {
        ...
        $st = $pdo->prepare(
            'INSERT INTO curriculum_courses (id, code, name, updated_at, payload)
             VALUES (?, ?, ?, ?, ?)
    ...
    if ($method === 'DELETE') {
        ...
        $st = $pdo->prepare('DELETE FROM curriculum_courses WHERE id = ?');
        $st->execute([$id]);
